...
class AzureOpenAIProvider(BaseLLMProvider):
    ...
    def _get_client(self):
        ...
        return openai.AsyncAzureOpenAI(
            ...
            azure_endpoint=self.base_url or settings.AZURE_OPENAI_ENDPOINT,
            api_version=self.config.get("api_version", settings.AZURE_OPENAI_API_VERSION),
    ...
    async def chat(self, messages, model, temperature=None, max_tokens=None,
        ...
        return self._make_oai_response(
            ...
            finish_reason=finish or "stop", tool_calls=tc,
        )
    ...
    async def embeddings(self, input_texts, model, **kwargs) -> OAIEmbeddingResponse:
        ...
        data = [OAIEmbeddingData(embedding=d.embedding, index=d.index) for d in response.data]
        return OAIEmbeddingResponse(
            data=data, model=response.model,
            usage=OAIEmbeddingUsage(prompt_tokens=response.usage.prompt_tokens,
                                    total_tokens=response.usage.total_tokens),
        )

    async def list_models(self) -> list[str]:
        return self.config.get("models", ["gpt-4o", "gpt-4o-mini"])
